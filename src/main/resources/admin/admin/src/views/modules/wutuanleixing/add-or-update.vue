<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="舞团类型" prop="wutuanleixing">
          <el-input v-model="ruleForm.wutuanleixing" 
              placeholder="舞团类型" clearable  :readonly="ro.wutuanleixing"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  data() {
	...
	wutuanleixing : false,
      ...
      rules: {
          wutuanleixing: [
                { required: true, message: '舞团类型不能为空', trigger: 'blur' },
          ],
      }
    };
  },
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `wutuanleixing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.wutuanleixingCrossAddOrUpdateFlag = false;
						 this.parent.search();
